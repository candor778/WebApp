...
export function ProjectCard({ project }: ProjectRowProps) {
  ...
  const handleExport = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      a.href = url
      a.download = `${project.project_id}_responses_${new Date()
        .toISOString()
        .split("T")[0]}.xlsx`
  ...
  return (
    <div
      ...
      className="group flex items-center gap-4 rounded-xl border-2 border-gray-200 px-4 py-3 hover:border-[#1b3750]/30 hover:bg-[#1b3750]/5 hover:shadow-md transition-all cursor-pointer"
    >
      {/* Left: Project Info */}
      ...
      {/* Responses - Highlighted with background */}
      <div className="flex items-center gap-2 min-w-[120px] px-3 py-1 rounded-lg bg-[#1b3750]/5 border border-[#1b3750]/10">
      ...
      >
        <Button 
          size="icon" 
          variant="outline"
          className="hover:bg-[#1b3750] hover:text-white hover:border-[#1b3750] transition-colors"
        >
        ...
        <Button
          ...
          className="hover:bg-blue-600 hover:text-white hover:border-blue-600 disabled:opacity-50 transition-colors"
        >
